...
static std::list<pickup::act_item> realloc_drop_locations( Character &ch,
        const drop_locations &items )
{
    std::list<pickup::act_item> ret;

    for( const drop_location &it : items ) {
        ret.emplace_back( pickup::act_item( it.loc, it.count, 0 ) );
    }

    return pickup::reorder_for_dropping( ch, ret );
}
...
{
    this->items = realloc_drop_locations( ch, items );
}
