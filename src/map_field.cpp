        ...
        for( int x = 0; x < my_MAPSIZE; x++ ) {
            for( int y = 0; y < my_MAPSIZE; y++ ) {
                if( field_cache[ x + y * MAPSIZE ] ) {
                    tripoint p( x, y, z );
                    submap *const current_submap = get_submap_at_grid( p );
                    if( current_submap == nullptr ) {
                        debugmsg( "Tried to process field at %s but the submap is not loaded", p.to_string() );
                        continue;
                    }
                    process_fields_in_submap( current_submap, p );
                }
            }
        }
    ...
    if( can_spread ) {
        ...
        for( size_t i = ( end_i + 1 ) % neighs.size(), count = 0;
            ...
            if( can_spread && rng( 1, 100 ) < spread_chance &&
                ...
                add_field( dst_p, fd_fire, 1, 0_turns, false );
                field_entry *tmpfld = dst.find_field( fd_fire );
                if( tmpfld != nullptr ) {
