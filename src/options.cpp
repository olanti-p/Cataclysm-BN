...
void options_manager::add_options_interface()
{
    ...
    std::vector<options_manager::id_and_option> lang_options = {
        { "", string_format( _( "System language [%s]" ), get_system_language().id ) }
    };
    for( const language_info &lang : list_available_languages() ) {
        lang_options.push_back( {lang.id, lang.disp_name()} );
    }
